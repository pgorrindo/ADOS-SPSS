...
* for autism spectrum disorder (ASD) classifications, using the original scoring algorithm, for all 4 ADOS modules.
* .
...
**************************************************.
**************************************************.
*first re-cast variables that were exported by REDCap, from strings to numerics.
**************************************************.
...
**************************************************.
**************************************************.
*now calculate autism (AU) and autism spectrum (ASD) classifications based on subdomain scores.
...
VARIABLE LABELS ados_classif_calc "ADOS classification - calculated".
...
**************************************************.
NUMERIC ados_classif_agree_manual_calc (F3.0).
VARIABLE LABELS ados_classif_agree_manual_calc "ADOS classification - agreement between calculated and manual".
VALUE LABELS ados_classif_agree_manual_calc 
	0 'do not agree'
	1 'agree'.
VARIABLE LEVEL ados_classif_agree_manual_calc (NOMINAL).
EXECUTE.

DO IF (MISSING(ados_classif2) <> 1).
  DO IF (ados_classif2 = ados_classif_calc).
    COMPUTE ados_classif_agree_manual_calc=1.
  ELSE IF (ados_classif2 <> ados_classif_calc).
    COMPUTE ados_classif_agree_manual_calc=0.
  END IF.
END IF.
EXECUTE.
...
CROSSTABS
  /TABLES=ados_classif_agree_manual_calc BY study_group
...
VARIABLE ATTRIBUTE VARIABLES=
  ados_classif_calc
  ados_classif2
  ados_classif_agree_manual_calc
  ados_module
...
SORT CASES BY
  include_flag (A)
  ados_classif_agree_manual_calc (D)
  ados_module (A).
